...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask=None, head_mask=None, output_attentions=False, gauss_weight=None):
        ...
        sa_output = sa_output + residual_x # (bs, seq_length, dim)

        # Feed Forward Network
        residual_sa = sa_output
        sa_output = self.output_layer_norm(sa_output)
        ffn_output = self.ffn(sa_output)  # (bs, seq_length, dim)
        ffn_output = ffn_output + residual_sa # (bs, seq_length, dim)
